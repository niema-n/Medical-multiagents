...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "version": API_VERSION,
        "graph_id": GRAPH_ID,
        "checkpointer": type(graph.checkpointer).__name__ if graph.checkpointer else "platform",
...
def consultation_start(payload: ConsultationStartRequest) -> dict[str, Any]:
    ...
    thread_id = _new_thread_id(payload.session_id)
    logger.info(
        "consultation_start_patient_case_received",
        extra={
            "thread_id": thread_id,
            "patient_id": payload.patient_id,
            "patient_name": payload.patient_name,
            "patient_case_preview": patient_case[:180],
        },
    )
